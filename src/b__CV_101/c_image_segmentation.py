import cv2
import numpy as np
from src.a__IP_Basics.utilities import imshow,describe,debugger,build_montages,print_h
...
def assignment():
	...
	#Input
	field = cv2.imread("Data/CV/drone_view.png")
	imshow("field",field)
	cv2.waitKey(0)
	cv2.destroyWindow("field")

	# Task Function
	plants_segmented = segment_plants(field)
	if (field==plants_segmented).all():
...
def main():
	...
	img = cv2.imread("Data/CV/boy_who_lived/vignette.jpg",cv2.IMREAD_ANYDEPTH)
