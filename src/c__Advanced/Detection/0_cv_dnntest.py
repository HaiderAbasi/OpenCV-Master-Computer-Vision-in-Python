...
classes = open('Data/NonFree/dnn/coco.names').read().strip().split('\n')
np.random.seed(42)
colors = np.random.randint(0, 255, size=(len(classes), 3), dtype='uint8')

# Give the configuration and weight files for the model and load the network.
net = cv.dnn.readNetFromDarknet('Data/NonFree\dnn\yolov3.cfg', 'Data/NonFree\dnn\yolov3.weights')
...
def load_image(path):
    ...
    cv.putText(img,f'forward propagation time={t:.3}',(50,50),cv.FONT_HERSHEY_PLAIN,1,(255,0,0),2)
...
def post_process(img, outputs, conf):
    ...
    if len(indices) > 0:
        for i in indices.flatten():
            ...
            cv.putText(img, text, (x, y - 5), cv.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
...
def trackbar(x):
    ...
    cv.putText(img,f'confidence level={conf}',(50,100),cv.FONT_HERSHEY_PLAIN,1,(255,0,0),2)
    #cv.displayOverlay('window', f'confidence level={conf}')
    cv.imshow('window', img)

cv.namedWindow('window')
cv.createTrackbar('confidence', 'window', 50, 100, trackbar)
load_image('Data\CV\drag_race.jpg')
