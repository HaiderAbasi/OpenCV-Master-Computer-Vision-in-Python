...
from multi_object_tracking_v2 import multitracker
        ...
        if (elasped_time % 1)< 0.2:
            ...
            if not self.futures.running():
                ...
                if self.identifying_faces:
                    ...
                    # Cases :
                    # Method : Voting based security
                    #          Screen will only be lock when all personnel present in the camera are voted
                    #          unauthorized 3 times. Otherwise, move normally
                    # TODO: 
                    #          Perform recognition after increasing intervals for each tracked personel
                    #          time_after_recognize = [4,8,16,32,64,128,248....] = [2^2+n] where n = 0,1,2,3,4,5
                    #          if face_iou>thresh and time_after_recognize elapsed:
                    #              recognize(face)
    ...
    def activate_sa(self,vid_id,dataset_dir =""):   
        ...
        while cap.isOpened():
            ...
            if config.display_state:
                ...
                cv2.putText(frame_draw,f"Future {self.future_calls} Calls made  with {len(children)} subprocess running!", ( 20,50 ) ,cv2.FONT_HERSHEY_PLAIN, 0.8, (255,128,128))
