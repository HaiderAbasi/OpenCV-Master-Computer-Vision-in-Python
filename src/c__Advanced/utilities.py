...
def build_montages(image_list, image_shape=None, montage_shape=None,titles=[],resize_to_default = True,draw_borders = False):
    ...
    if montage_shape == None:
        #montage_rows = (len(image_list)//4) + 1
    ...
    for idx,img in enumerate(image_list):
        ...
        if len(titles)!=0:
            putText(img,titles[idx],(20,20),cv2.FONT_HERSHEY_PLAIN,1,(255,255,255),1)
...
def imshow(winname,img,WindowType = cv2.WINDOW_NORMAL,check_alpha = False, resize = False):
    """
    (displays complete image on the screen.)

    Parameters:
    winname (String):    Name of the image
    ...
    if (check_alpha and (len(img.shape)==4)):
    ...
    if (resize and ((img_disp.shape[0]>=480) or (img_disp.shape[1]>=640))):
        img_disp = cv2.resize(img_disp,None,fx=0.5,fy=0.5)


    cv2.namedWindow(winname,WindowType)
    cv2.setWindowProperty(winname, cv2.WND_PROP_TOPMOST, 1) # Reference: https://stackoverflow.com/a/66364178/11432131
