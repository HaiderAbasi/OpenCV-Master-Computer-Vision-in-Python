...
def download_missing_yolo_model_files(deepsort_dir):
    """Download missing model files from Google Drive."""
    models_dir = os.path.join(deepsort_dir, 'yolov5', 'weights')
    model_files = ['yolov5s.pt', 'yolov5m.pt']  # replace with actual file names
    files_id = ['180VTDW-W2Q871l5PdPZGbf-SzP1KVIzm', '1rJf16KqhIuq6_5NDNLiwEIAKAx-fS81k']  # replace with actual file IDs or URLs
...
def download_missing_model_files(deepsort_dir):
    """Download missing model files from Google Drive."""
    models_dir = os.path.join(deepsort_dir, 'deep_sort', 'deep','checkpoint')
    model_files = ['ckpt.t7']  # replace with actual file names
    files_id = ['1kwwSf3uF7_FQid_XywQGfysVOpoFzFYr']  # replace with actual file IDs or URLs
